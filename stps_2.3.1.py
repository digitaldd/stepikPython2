...
только по таким элементам x из последовательности a, что f(x) равно True. Будем говорить, что в этом случае функция f
...
class multifilter:
    ...
    def generFuncs(self):
        for i in range(len(self.list)):
            ...
            if self.judge(checkList.count(True), checkList.count(False)):
                self.answer.append(self.list[i])
            yield self.answer
    ...
    def __init__(self, iterable, *funcs, judge=judge_any):
        ...
        self.a = 0
        self.judge = judge
        self.gen = multifilter.generFuncs(self)
    ...
    def __next__(self):
        if self.a < len(self.answer):
            self.a += 1
            return self.answer[self.a - 1]
        raise StopIteration
