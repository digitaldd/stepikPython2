...
API https://developers.artsy.net/start и выполнить необходимые шаги, а именно зарегистрироваться, создать приложение,
...
на сайте используется кодировка UTF-8, что может привести к ошибке при попытке записать в файл имя с
необычными символами. Вы можете использовать print, или аргумент encoding функции open.
'''
...
token = "eyJhbGciOiJIUzI1NiJ9.eyJyb2xlcyI6ImFydHN5Iiwic3ViamVjdF9hcHBsaWNhdGlvbiI6IjVkNDA5OTZlNmU2MDQ5MDAwNzQ5MGZhMi" \
        "IsImV4cCI6MTYwNTQ0NTY5NywiaWF0IjoxNjA0ODQwODk3LCJhdWQiOiI1ZDQwOTk2ZTZlNjA0OTAwMDc0OTBmYTIiLCJpc3MiOiJHcmF" \
        "2aXR5IiwianRpIjoiNWZhN2VkYzEwZmNkMmMwMDE0ZDQ1MmU1In0.8j84KkEVqrcUzZEMpTiHAMsAtv_gwdXoDWxYSzHEoRw"
headers = {"X-Xapp-Token": token}
listAnwer = []
for line in sys.stdin:
    r = req.get("https://api.artsy.net/api/artists/" + str(line).strip(), headers=headers)
    listAnwer.append(dict(js.loads(r.text)).get("birthday") + " " + dict(js.loads(r.text)).get("sortable_name"))
listAnwer.sort()
for i in listAnwer: print(i[5:])
