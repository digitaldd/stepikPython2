...
def create(name, arg):
    global env
    env.update({name: {"parent": env[arg], "vars": set()}})


def add(name, arg):
    global env
    env[name]["vars"].add(arg)


def get(name, arg):
    global env
    global listAnswer
    if (env[name]["vars"].__contains__(arg)):
        listAnswer.append(name)
        return
    if (name == "global") & (not env[name]["vars"].__contains__(arg)):
        listAnswer.append(None)
        return
    else:
        get(list(env.keys())[list(env.values()).index(env[name]["parent"])], arg)  # find the key from value in the dict


env = {"global": {"parent": None, "vars": set()}}
listAnswer = []
for i in range(int(input())):
    command, name, arg = input().split()
    if command == "add":
        add(name, arg)
